	...
	public LinkedHashMap<User, BigDecimal> getAllUsersWithFines(List<User> users){
		...
		for (User user : users) {
			...
			if (user.getBooks().size() == 0) {
				...
				BigDecimal totalFineInRupes = fineAmount.multiply(totalDaysTooLateInBD); 
				allUsersWithFines.put(user, totalFineInRupes);
			}
		}	
	...
	public LinkedHashMap<Book, BigDecimal> getBooksWithFines(List<Book> books){
		...
		for(Book book : books) {
			...
			if (daysTooLate > 0) {
				...
				BigDecimal totalFineInRupes = fineAmount.multiply(daysTooLateInBD);
				booksWithFines.put(book, totalFineInRupes);
			} else {
